...
const TutorDashboardHome = () => {
  ...
  const recentActivities = [
    {
      icon: BookOpen,
      text: `${stats.totalSessions || 0} sessions created`,
      color: "text-indigo-600",
    },
    {
      icon: FileText,
      text: `${stats.totalMaterials || 0} materials uploaded`,
      color: "text-green-600",
    },
    {
      icon: Users,
      text: `${stats.totalStudents || 0} students enrolled`,
      color: "text-blue-600",
    },
    {
      icon: Star,
      text: `Average rating: ${stats.avgRating?.toFixed(1) || 0} stars`,
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        >
          ...
          <motion.p
            ...
            className="text-lg text-base-content/70 mt-3 font-medium"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {statCards.map((card, idx) => (
            ...
            >
              <div
                className={`bg-gradient-to-r ${card.gradient} text-white p-6 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 flex items-center gap-4 h-full border border-white/20`}
              >
                <motion.div
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  ...
                  className="p-3 bg-white/20 rounded-xl group-hover:scale-110 transition-transform"
                >
                  {(() => {
                ...
                <div className="flex-1">
                  <h3 className="text-sm md:text-base font-medium opacity-90">
                    {card.title}
                  </h3>

                  <p className="text-3xl md:text-4xl font-extrabold mt-2 flex items-baseline">
                    {card.prefix && (
                      <span className="text-xl mr-1">{card.prefix}</span>
                    )}
        ...
        <motion.section
          ...
          className="bg-base-100 text-base-content rounded-2xl shadow-lg p-6 md:p-8 border border-base-300"
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {quickActions.map((action, idx) => (
              ...
              >
                <Link
                  to={action.link}
                  className="flex flex-col items-center justify-center p-6 rounded-xl border border-base-300 bg-base-200 hover:bg-base-300 transition-all cursor-pointer group h-full"
                >
                  {(() => {
                    const Icon = action.icon;
                    return (
                      <Icon
                        className={`w-10 h-10 ${action.iconClass} mb-2 group-hover:scale-110 transition-transform`}
                      />
                    );
                  })()}
          ...
          <div className="space-y-4">
            {recentActivities.map((item, idx) => (
              <motion.div
                ...
                transition={{ delay: idx * 0.1 }}
                className="flex items-center gap-3 p-3 rounded-lg bg-base-200 hover:bg-base-300 transition-colors border border-base-300"
              >
